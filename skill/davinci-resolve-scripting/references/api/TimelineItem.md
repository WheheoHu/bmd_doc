...
Returns a string with TimelineItem's audio mapping information. Check '[Audio Mapping](../settings/AudioMapping.md)' section below for more information.
...
Returns the clip's [color group](../api/ColorGroup.md) if one exists.
...
Exports LUTs from tiItem referring to value passed in 'exportType' (enum) for LUT size. Refer to. [ExportLUT notes](../settings/ExportLUT.md) section for possible values.
