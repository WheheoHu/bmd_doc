...
Returns a dict of project names in the current folder mapped to their attributes. Refer to section [Project Attributes](../resolve_settings/ProjectAttributes.md) for the supported keys.
...
###  CreateProject(projectName, mediaLocationPath)
Return Type: `Project`

Creates and returns a project if projectName(string) is unique, and None if it is not. Accepts an optional argument `mediaLocationPath` to set the media location path.
