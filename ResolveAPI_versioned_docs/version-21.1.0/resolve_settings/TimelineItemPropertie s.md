...
---
This section covers additional notes for the functions [`TimelineItem:GetProperties`](../resolve_api/TimelineItem.md#getproperties) and [`TimelineItem:SetProperties`](../resolve_api/TimelineItem.md#setpropertiesproperties). These functions are used to get and set the item properties otherwise available to the user through the Inspector.

The arguments are passed as a dictionary (for python) or a table (for lua) of key and value pairs.

The single-key forms [`TimelineItem:GetProperty`](../resolve_api/TimelineItem.md#getpropertypropertykey) and [`TimelineItem:SetProperty`](../resolve_api/TimelineItem.md#setpropertypropertykey-propertyvalue) accept the same keys, but are a [deprecated calling convention](../other/misc.md#deprecated-calling-conventions) since 21.1.0.
