...
1. Sample Workflow Integration Plugin
   a. Demonstrates some basic functions that can be performed in DaVinci Resolve using the scripting APIs.
   b. Uses latest Electron process sandboxed model and context isolation in the plugin implementation, recommended for plugin writers.
   c. Available in the "Examples/SamplePlugin" directory.

2. Sample Workflow Integration Plugin (Promise apis)
   a. Demonstrates the usages of scripting promise apis to invoke apis asynchronously.
   b. Uses latest Electron process sandboxed model and context isolation in the plugin implementation, recommended for plugin writers.
   c. Available in the "Examples/SamplePromisePlugin" directory.

3. Sample Workflow Integration Plugin (Compatibility Mode)
   a. A sample plugin to assist plugin developers who have developed plugins for DaVinci Resolve 19.0.1 or older versions and use the legacy non-sandboxed model in their plugin implementation.
   b. Compatible with old and new Electron versions.
   c. Demonstrates some basic functions that can be performed in DaVinci Resolve using the scripting APIs.
   d. Available in the "Examples/CompatibleSamplePlugin" directory.
   e. The non-sandboxed approach is not recommended by Electron and the example is only meant to demonstrate how an existing plugin can be easily modified to work with the new version of Electron.
   f. In particular, note the following settings in "Examples/CompatibleSamplePlugin/main.js":
      - nodeIntegration
      - contextIsolation
   g. Also refer to "Examples/CompatibleSamplePlugin/preload.js" to see how "WorkflowIntegration.node" is loaded and Resolve scripting APIs are accessed.

4. Script Test Workflow Integration Plugin
   a. To run, test or simulate workflows using scripts.
   b. There are few Workflow Integrations sample scripts present in the "Scripts" folder which could be run using this plugin.
   c. Available in the "Examples/ScriptTestPlugin" directory.
...
WorkflowIntegration
  GetInfo()                                       --> Object             # Returns an object containing following module information:
                                                                           - version : string containing WorkflowIntegration module version (example: '2.0.0')
  ...
  InitializePromise(<pluginId>)                   --> <Promise>          # Fulfills upon successful initialization with true, false otherwise. <pluginId> is the unique plugin id string configured in the manifest.xml file.
  GetResolve()                                    --> Resolve            # Returns Resolve object.
  GetResolvePromise()                             --> <Promise>          # Fulfills upon success with Resolve object. All further calls to the returned Resolve object will, in turn, return the promise objects.
...
Basic Resolve API
-----------------
Please refer to the [Basic Resolve API] section in '../Developer/Scripting/README.md' file for the list of the functions that Resolve offers for scripted control. For plugin scripts, the 'resolve' and 'project' variables are automatically set up for you, and may be used to access any part of Resolve's API.
