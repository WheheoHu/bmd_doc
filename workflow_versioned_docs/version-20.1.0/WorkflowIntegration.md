:::note
Updated as of 3 October, 2024 (framework notes for Resolve 20.1 from the Workflow Integrations changelog, 28 July 2025)
:::
...
- Resolve 20.1 updates the bundled Electron to 36.3.2, improves handling of large lists and data collections in JavaScript based workflows, and addresses unresponsive queries while a DaVinci Resolve modal dialog is active.
...
2. Sample Workflow Integration Plugin (Promise apis) — new in 20.1.0
   - Demonstrates the usages of scripting promise apis to invoke apis asynchronously.
   - Uses latest Electron process sandboxed model and context isolation in the plugin implementation, recommended for plugin writers.
   - Available in the "Examples/SamplePromisePlugin" directory.
...
Returns an object containing the following module information:

- `version`: string containing the WorkflowIntegration module version (example: "2.0.0")
...
### InitializePromise(pluginId)

> New in 20.1.0

Returns a `Promise`. Fulfills upon successful initialization with true, false otherwise. `pluginId` is the unique plugin id string configured in the manifest.xml file.
...
Returns a `Promise`. Fulfills upon success with the Resolve object. All further calls to the returned Resolve object will, in turn, return promise objects.

This is the asynchronous counterpart to [Initialize](#initializepluginid) and [GetResolve](#getresolve). See the "Examples/SamplePromisePlugin" directory for a working plugin built on these APIs.

```javascript
    const WorkflowIntegration = require('./WorkflowIntegration.node');
    const isInitialized = await WorkflowIntegration.InitializePromise('com.blackmagicdesign.resolve.samplepromiseplugin');
    if (isInitialized) {
        const resolve = await WorkflowIntegration.GetResolvePromise();
        const projectManager = await resolve.GetProjectManager();
        await projectManager.CreateProject("Hello World");
    }
```
